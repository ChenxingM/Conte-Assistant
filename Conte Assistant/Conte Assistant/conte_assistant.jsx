...
function checkScriptSet(){
    if (!isSecurityPrefSet()) {
        alert("このスクリプトはファイルを書き込むためにアクセス権限が必要です。環境設定にスクリプトとエクスプレッションを選択し、「スクリプトによるファイルへの書き込みとネットワークへのアクセスを許可」オプションを選択してください！",scriptName);
        try {
...
function getIconsFolder() {
    ...
    var aescriptsFolder = Folder(userDataFolder.toString() + "/Conte Assistant/Icons");
    if (!aescriptsFolder.exists) {
        var checkFolder = aescriptsFolder.create();
        if (!checkFolder) {
            alert("フォルダー作りエラー！",scriptName);
...
for(var i in settings){
    switch (i) {
        ...
        case "Title":
            {titleName_txt.text = settings[i];}
            break;
        case "Epsd":
            {epsNum_txt.text = settings[i];}
            break;
        case "PART":
            {part_edrtx.text = settings[i];}
            break;
        case "Sence":
            {sence_edtxt.text = settings[i];}
            break;
        case "Cut":
            {cno_edtxt.text = settings[i];}
            break;
        case "resW":
            {res_width_edtxt.text = settings[i];}
            break;
        case "resH":
            {res_height_edtxt.text = settings[i];} 
            break;
        case "sendToAME":
            {if(settings[i]){Export.text = "Send To AME";}
             else{Export.text = "Export in AE";}}       
            break;       
        case "second":
            {time_sec.text = settings[i];} 
            break;    
        case "frame":
            {time_frm.text = settings[i];} 
            break;  
        case "FPS":
            {fmrat_edtxt.text = settings[i];} 
            break;
        case "charaN":
            {charaName_txt.text = settings[i];} 
            break;     
        case "charaS":
            {serifu_edtxt.text = settings[i];} 
             break; 
        case "charaC":
            {myColor = settings[i];} 
            break;      
...
getColor_btn.onClick = get_color;
function get_color(){
...
function addTxt(txtd){
    ...
    textProp1.expression = 'text.sourceText.style.setFillColor([0,0,0]).setFontSize(120).setFont("MS-Gothic").setStrokeColor([1,1,1]).setStrokeWidth(10).setApplyStroke(1);';    
...
// =====
    ...
    version.text = "Version: 0.3 alpha (Demo)"; 
    ...
    upDate.text = "Last Update: 2023/01/22"; 
    ...
    notice.add("statictext", undefined, "This is a demo version After Effect script. "); 
...
function serifu_it(){ 
    ...
    textProp1.expression = 'text.sourceText.style.setFillColor([' + String(myColor) + ']).setFontSize(45).setFont("MS-Gothic").setStrokeColor([1,1,1]).setStrokeWidth(5).setApplyStroke(1);';
    ...
    textProp2.expression = 'text.sourceText.style.setFillColor([0,0,0]).setFontSize(45).setFont("MS-Gothic").setStrokeColor([1,1,1]).setStrokeWidth(3).setApplyStroke(1);';
