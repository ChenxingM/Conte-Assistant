...
var cutDD = cuts_group1.add("dropdownlist", undefined, "无卡信息");
    cutDD.preferredSize.width = 150;

//var exportButton = cutSlc_pnl.add("button", undefined, "Export Layer Info");
var importButton = cutSlc_pnl.add("button", undefined, "读取卡信息");
var deleteInfoButton = cutSlc_pnl.add("button", undefined, "删除全部"); 
    ...
    for(var i in lsettings){
        switch (i) {
            ...
            case "resH":
                ...
                break;
            case "sendToAME":
                {if(lsettings[i]){Export.text = "AME导出";}
                else{Export.text = "AE导出";}}       
                break;       
            case "second":
...
function changeSettings(){
    ...
    var fontTxt = setWindow.add("statictext",undefined,"字体");
    ...
    var fontFile = File(psFolder + "/fonts.txt");
    //var pathToPs1File = psFolder+'/getFonts.ps1';
    if(!fontFile.exists){
       getFont();
    }
    ...
    fontFile.close();
  //  loadSettings();
    fontsDD.selection = fontsDD.find(userFont.toString());
    ...
    ok_btn.onClick = function(){
        ...
        if($.os.indexOf("Windows") != -1){settings["font"] = userFont = "MS-Gothic";}
        else {settings["font"] = fontsDD.selection.toString();userFont = fontsDD.selection;}

        cutAddOne = addOne.value;
        ...
        //export button text
        if(settings["sendToAME"]) Export.text = "AME导出";
        else Export.text = "AE导出";
...
function expLayerInfo() {
    try {
        ...
        if(containsNonAsciiCharacters(infoText)) {
            //alert("请勿使用全角字符",scriptName);
        }
        else{         
            // layer info string
...
function importLayerInfo(infoText) {
    ...
    try {
        ...
        if (file.exists) {
            ...
            else alert("相机图层不能时间重映射，请用模版工程重新创建", scriptName);
            ...
            alert("读取完成!",scriptName);
        }else alert("卡信息不存在！", scriptName);
    } catch (error) {
        alert("An error occurred: " + error.toString(), scriptName);
    }
};
